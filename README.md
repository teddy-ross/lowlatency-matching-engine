...
`std::expected`, `std::print`/`std::println` (with a `std::format` fallback on pre-GCC-14 stdlibs), `std::unreachable`, static `operator()`, `std::to_underlying`, concepts (`EventSink`, `OrderRange`), `std::pmr` pooled containers, designated initializers, `std::from_chars`, `std::source_location` (test harness), `std::format_to`.
...
`tests/engine_tests.cpp` pins down matching semantics (maker-price execution, FIFO time priority, level sweeping, cancel paths, rejects), the parser's error taxonomy, the exact `DUMP` format, and the custom-sink API — framework-free, with `std::source_location` failure reporting that survives Release builds.
